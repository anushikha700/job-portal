...
<head>
	...
	<!--//Style-CSS -->
        ...
        <script type="text/javascript">
            ...
            function varifyUsername(x, y) {
                ajax = new XMLHttpRequest();

                ajax.open("GET", "../JsController?op=varify2&username=" + x, true);
           ...
           function varifyEmailid(x, y) {
                ...
                ajax.open("GET", "../JsController?op=varify1&email=" + x, true);
				...
				<div class="form-left">
					<div class="middle">
						...
						<p>Create Your Account, It's Free.</p>
                                    ...
                                    <form action="..\JsController?op=add" method="post" class="signin-form" onsubmit="return comparePwds(password.value,rpassword.value);">
					
							<div class="form-input">
								<label>Email</label>
								<input type="email" name="email" placeholder="" value="${jobseeker.email}" required="required" autocomplete="off" onblur="varifyEmailid(this.value, s1);"/>
							</div>
							<div class="form-input">
								<label>Username</label>
								<input type="text" name="username" placeholder="" value="${jobseeker.username}" required="required" autocomplete="off" onblur="varifyUsername(this.value, s1);"/>
							</div>
							<div class="form-input">
								<label>Password</label>
								<input type="password" name="password" placeholder="" value="${jobseeker.password}" required="required" autocomplete="off" pattern="(?=^.{8,}$)((?=.*\d)|(?=.*\W+))(?![.\n])(?=.*[A-Z])(?=.*[a-z]).*$"/>
							</div>
                                                        <div class="form-input">
								<label>Confirm Password</label>
								<input type="password" name="rpassword" placeholder="" value="${jobseeker.password}" required="required" autocomplete="off" />
							</div>
